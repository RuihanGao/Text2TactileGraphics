    ...
    function setDownloadLabel(link, baseLabel, fileUrl) {
      fetch(fileUrl, { method: "HEAD", cache: "force-cache" })
        .then(function (res) {
          ...
          link.setAttribute(
            "aria-label",
            baseLabel + " for " + r.object + " (" + formatBytes(bytes) + ")",
          );
        })
        .catch(function () {
  ...
  function updateCounter(swiper) {
    ...
    if (counterRegion) {
      counterRegion.setAttribute(
        "aria-label",
        "Showing result " + current + " of " + results.length,
      );
    }
    loadNearby(swiper);
  }
...
(function () {
  ...
  const tabs = Array.prototype.slice.call(document.querySelectorAll("#method-tabs .pipeline-step"));
  if (!tabs.length) return;
  const STARTS = tabs.map(function (el) {
    return Number(el.dataset.start);
  });
...
/* exported copyBibtex */ /* called from an inline onclick in index.html */
function copyBibtex() {
